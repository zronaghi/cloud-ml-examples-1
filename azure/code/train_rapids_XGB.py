...
def main():
    ...
    parser.add_argument('--num_boost_round', type=int, default=10, help='Number of rounds for boosting')
    parser.add_argument('--max_depth', type=int, default=6, help='Max depth of each tree')
    parser.add_argument('--learning_rate', type=float, default=0.3, help='Boosting learning rate')

    args = parser.parse_args()
    data_dir = args.data_dir

    num_boost_round = args.num_boost_round
    run.log('num_boost_round', np.int(args.num_boost_round))
    max_depth = args.max_depth
    run.log('max_depth', np.int(args.max_depth))
    learning_rate = args.learning_rate
    run.log('learning_rate', np.str(args.learning_rate))

    print('\n---->>>> cuDF version <<<<----\n', cudf.__version__)
    print('\n---->>>> cuML version <<<<----\n', cuml.__version__)

    compute = 'multi-GPU' # 'multi-GPU' option for using multi-GPU algorithms via Dask
    azure_ml = RapidsCloudML(cloud_type = 'Azure', model_type = 'XGBoost', data_type ='Parquet', compute_type = compute)
    print(compute)
